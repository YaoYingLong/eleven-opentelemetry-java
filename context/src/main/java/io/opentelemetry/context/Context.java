...
/**
 * 一种上下文传播机制，可以跨 API 边界和线程之间携带 scoped-values。
 *
 * A context propagation mechanism which can carry scoped-values across API boundaries and between
 * threads.
 *
 * 一个Context对象能够通过makeCurrent设置到ContextStorage，有效的构成了context scope，scope是被绑定到当前线程的
 * 在scope范围内，Context甚至可以跨 API 边界访问，可以通过Scope.close方法关闭scope
 *
 ...
 *
 * Context对象是不可变的，且从父Context中继承state，添加或重写Context当前的state需要新建一个Context对象
 * 然后添加替换之前的Context
 *
