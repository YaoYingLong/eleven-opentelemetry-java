...
public final class SdkMeterProviderUtil {
   ...
   * @param sdkMeterProviderBuilder the builder
   */
  public static void registerMetricReaderWithCardinalitySelector(SdkMeterProviderBuilder sdkMeterProviderBuilder,
      MetricReader metricReader, CardinalityLimitSelector cardinalityLimitSelector) {
    // 这里传入的MetricReader默认是PeriodicMetricReader，默认包装的MetricExporter是OtlpGrpcMetricExporter
    try {
      // 通过反射的方式调用SdkMeterProviderBuilder的registerMetricReader方法，将MetricReader和CardinalityLimitSelector设置到HashMap中
