...
public final class PeriodicMetricReader implements MetricReader {
  ...
  // CollectionRegistration默认是SdkCollectionRegistration
  ...
  PeriodicMetricReader(MetricExporter exporter, long intervalNanos, ScheduledExecutorService scheduler) {
    ...
    this.scheduled = new Scheduled(); // 创建一个真正执行export的线程
  }
  ...
  // 这里默认注册是SdkCollectionRegistration
  @Override
  ...
  void start() {
    synchronized (lock) {
      ...
      // 周期执行Scheduled线程，实际是周期执行Scheduled的doRun，周期默认为1分钟
